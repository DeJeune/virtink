...
func (r *ControllerExpectations) DeleteExpectations(controllerKey string) {
	if exp, exists, err := r.GetByKey(controllerKey); err == nil && exists {
		if err := r.Delete(exp); err != nil {
			log.Log.V(2).Info(fmt.Sprintf("Error deleting expectations for controller %v: %v", controllerKey, err))
		}
	}
}
...
func (r *ControllerExpectations) SatisfiedExpectations(controllerKey string) bool {
	if exp, exists, err := r.GetExpectations(controllerKey); exists {
		if exp.Fulfilled() {
			log.Log.V(4).Info(fmt.Sprintf("Controller expectations (name: %s) fulfilled %#v", r.name, exp))
			return true
		} else if exp.isExpired() {
			log.Log.V(4).Info(fmt.Sprintf("Controller expectations (name: %s) expired %#v", r.name, exp))
			return true
		} else {
			log.Log.V(4).Info(fmt.Sprintf("Controller (name: %s) still waiting on expectations %#v", r.name, exp))
			return false
		}
	} else if err != nil {
		log.Log.V(2).Info(fmt.Sprintf("Error encountered while checking expectations (name: %s) %#v, forcing sync", r.name, err))
	} else {
		...
		log.Log.V(4).Info(fmt.Sprintf("Controller %v (name: %s) either never recorded expectations, or the ttl expired.", controllerKey, r.name))
	}
...
func (r *ControllerExpectations) SetExpectations(controllerKey string, add, del int) {
	...
	log.Log.V(4).Info(fmt.Sprintf("Setting expectations %#v", exp))
...
func (r *ControllerExpectations) LowerExpectations(controllerKey string, add, del int) {
	if exp, exists, err := r.GetExpectations(controllerKey); err == nil && exists {
		...
		log.Log.V(4).Info(fmt.Sprintf("Lowered expectations %#v", exp))
	}
}
...
func (r *ControllerExpectations) RaiseExpectations(controllerKey string, add, del int) {
	if exp, exists, err := r.GetExpectations(controllerKey); err == nil && exists {
		exp.Add(int64(add), int64(del))
		// The expectations might've been modified since the update on the previous line.
		log.Log.V(4).Info(fmt.Sprintf("Raised expectations %#v", exp))
	}
}
...
func (u *UIDTrackingControllerExpectations) ExpectDeletions(rcKey string, deletedKeys []string) {
	...
	if existing := u.GetUIDs(rcKey); existing != nil && existing.Len() != 0 {
		log.Log.Error(fmt.Errorf("clobbering existing delete keys"), "keys", existing)
	}
	expectedUIDs := sets.NewString()
	for _, k := range deletedKeys {
		expectedUIDs.Insert(k)
	}
	log.Log.V(4).Info(fmt.Sprintf("Controller %v waiting on deletions for: %+v", rcKey, deletedKeys))
...
func (u *UIDTrackingControllerExpectations) AddExpectedDeletion(rcKey string, deletedKey string) {
	...
	log.Log.V(4).Info(fmt.Sprintf("Controller %v waiting on deletions for: %+v", rcKey, expectedUIDs))
	if err := u.uidStore.Add(&UIDSet{expectedUIDs, rcKey}); err != nil {
		panicWithKeyFuncMsg(err)
	}
	u.ControllerExpectationsInterface.ExpectDeletions(rcKey, expectedUIDs.Len())
...
func (u *UIDTrackingControllerExpectations) DeletionObserved(rcKey, deleteKey string) {
	...
	if uids != nil && uids.Has(deleteKey) {
		log.Log.V(4).Info(fmt.Sprintf("Controller %v received delete for pod %v", rcKey, deleteKey))
...
func (u *UIDTrackingControllerExpectations) DeleteExpectations(rcKey string) {
	...
	if uidExp, exists, err := u.uidStore.GetByKey(rcKey); err == nil && exists {
		if err := u.uidStore.Delete(uidExp); err != nil {
			log.Log.V(2).Info(fmt.Sprintf("Error deleting uid expectations for controller %v: %v", rcKey, err))
		}
	}
}
